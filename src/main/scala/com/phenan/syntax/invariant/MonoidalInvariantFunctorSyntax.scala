...
object MonoidalInvariantFunctorSyntax {
  def discardLeft [F[_], T] (fu: F[Unit], ft: F[T])(using monoidalInvariant: Monoidal[F] & InvariantFunctor[F]): F[T] = {
  ...
  def discardRight [F[_], T] (ft: F[T], fu: F[Unit])(using monoidalInvariant: Monoidal[F] & InvariantFunctor[F]): F[T] = {
